...
(use-package ht :ensure t :demand t)
(use-package dash :ensure t :demand t)
(use-package loop :ensure t :demand t)
(use-package s :ensure t :demand t)
(use-package crux :ensure t :demand t)
(use-package async
  :ensure t :demand t
  :commands (async-start))
(use-package load-relative :ensure t :demand t)
...
(let ((mise-shims (expand-file-name "~/.local/share/mise/shims")))
  ...
  (setq exec-path (append exec-path (list mise-shims))))
...
(use-package consult-xref-stack
  :ensure t
  (:host github :repo "brett-lempereur/consult-xref-stack")
  :after consult
...
(use-package kind-icon
  ...
  ;; TODO:
  ;; (add-to-list 'corfu-margin-formatters #'my/kind-icon-with-source)
...
(use-package corfu
  :after (dabbrev tempel tempel-collection)
  :defer 0.5
  :custom
  (corfu-auto t)
...
(after-packages (cape tempel corfu)
  ...
  (defun my/setup-cape-elisp-block ()
    "Add cape-elisp-block to completion-at-point-functions."
    (add-hook 'completion-at-point-functions #'cape-elisp-block nil t))

  (add-hook 'org-mode-hook #'my/setup-cape-elisp-block)
  (add-hook 'markdown-mode-hook #'my/setup-cape-elisp-block)

  ;; Various modes: add tempel and cape functions
  (defun my/setup-tempel-capf ()
    "Add tempel-complete to completion-at-point-functions."
    (add-hook 'completion-at-point-functions #'tempel-complete nil t))

  ;; Remove python-completion-at-point in python buffers (prefer eglot)
  (defun my/remove-python-capf ()
    "Remove python-completion-at-point to avoid duplicates with eglot."
    (remove-hook 'completion-at-point-functions #'python-completion-at-point t))

  (add-hook 'conf-mode-hook #'my/setup-tempel-capf)
  (add-hook 'prog-mode-hook #'my/setup-tempel-capf)
  (add-hook 'text-mode-hook #'my/setup-tempel-capf)
...
(use-package eat
  :disabled
  :ensure t
  (:host "www.codeberg.org" :repo "akib/emacs-eat.git")
  :defer 1
  :hook
...
(use-package ob-gptel
  :ensure t
  (:host github :repo "jwiegley/ob-gptel")
  :after org
...
(use-package marginalia
  ...
  (define-advice marginalia--buffer-file (:around (fn &rest args) my/project-relative)
    (let ((path (apply fn args)))
      (if-let* ((project-root (marginalia--project-root))
...
Only works in python-base-mode and derived modes."
  (interactive "r")
  (unless (derived-mode-p 'python-base-mode)
    (user-error "This command only works in Python mode"))
  (save-excursion
    (save-restriction
...
(use-package typst-preview
  :ensure t
  (:host github :repo "havarddj/typst-preview.el" :rev :newest)
  :after websocket
  :defer t
  :custom
...
function run() {
  ...
  const finished = list.filter(function(e) { return t(e.endDate) <= nowT; });
...
(defun my/calendar--fetch ()
  "Run the JXA script and return (:current C :next N :last L), or `no-access'.
C, N, and L are strings or nil."
  (let* ((out (with-temp-buffer
    ...
    (if (member "NO_ACCESS" lines)
        'no-access
      (cl-flet ((extract (tag) (let* ((prefix (concat tag ":"))
                                      (line (seq-find (lambda (l) (string-prefix-p prefix l)) lines))
                                      (v (and line (substring line (length prefix)))))
                                 (unless (equal v "NONE") v))))
        (list :current (extract "CURRENT")
              :next (extract "NEXT")
              :last (extract "LAST"))))))

(defun my/calendar--get (key fallback-message)
  "Return the event title for KEY from the calendar fetch, or nil.
Show FALLBACK-MESSAGE interactively when there's no event."
  (let ((result (my/calendar--fetch)))
    (if (eq result 'no-access)
        (progn
          ...
          nil)
      (let ((title (plist-get result key)))
        (when (called-interactively-p 'interactive)
          (message "%s" (or title fallback-message)))
        title))))

(defun my/calendar-current-event ()
  "Return the title of the currently active macOS Calendar event, or nil."
  (interactive)
  (my/calendar--get :current "No current event"))

(defun my/calendar-next-event ()
  "Return the title of the next upcoming macOS Calendar event, or nil."
  (interactive)
  (my/calendar--get :next "No upcoming events in the next 7 days"))

(defun my/calendar-last-event ()
  "Return the title of the most recently finished macOS Calendar event, or nil."
  (interactive)
  (my/calendar--get :last "No recently finished event"))
...
(defun my/calendar-insert-last-event ()
  "Insert the most recently finished macOS Calendar event title at point."
  (interactive)
  (if-let* ((title (my/calendar-last-event)))
      (insert title)
